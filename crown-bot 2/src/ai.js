...
async function callGemini(system, history, userText) {
  ...
  // A customer can easily send two messages before we reply, so collapse any
  // consecutive same-role turns into one before sending.
  const raw = [
  ...
  // Must open on a user turn; drop any leading model turns.
